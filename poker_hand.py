...
class Player:
    ...
    def get_dealer_cards(self, cards:tuple):
        ...
        self.characters.sort()
        self.patterns.sort()
    def get_dealer_cards(self, cards:list):
        for card in cards:
            if(card.character!=None):
                self.cards.append(card)
                self.characters.append(character_to_number(card.character))
                self.patterns.append(card.pattern)
        self.characters.sort()
...
def is_straight(player:Player):
    for i in range(0,len(player.characters)-5):
        if(player.characters[i]+4
        ==player.characters[i+1]+3
        ==player.characters[i+2]+2
        ==player.characters[i+3]+1
        ==player.characters[i+4]):
            return True
...
def win_lose(player1:Player, player2:Player):
    draw = 'draw'
    player1result=player1.name
    player2result=player2.name
    if(get_rank(player1)>get_rank(player2)):
        ...
        if(len(player1.characters)>=5 and len(player2.characters)>=5):
            if(get_rank(player1)==Rank.Straight_Flush):
                if(player1.characters[0]>player2.characters[0]):
                    ...
                    return draw
            elif(get_rank(player1)==Rank.Four_of_a_Kind):
                if(player1.characters[3]>player2.characters[3]):
                    return player1result
                elif(player1.characters[3]<player2.characters[3]):
                    return player2result
                else:
                    return draw
            elif(get_rank(player1)==Rank.Full_House):
                _, p1=is_three_of_a_kind(player1)
                _, p2=is_three_of_a_kind(player2)
                if(p1>p2):
                    return player1result
                elif(p1<p2):
                    return player2result
        ...
        if(len(player1.characters)>=1 and len(player2.characters)>=1):
            if(get_rank(player1)==Rank.High_Card):
                if(character_to_number("A") in player1.characters):
                    if(character_to_number("A") in player2.characters):
                        return draw
                    else:
                        return player1result
                else:
